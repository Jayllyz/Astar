...
elements *ll_find(linked_list_t *list, points point)//chercher un point
{
    linked_node_t *i = list->head;
    while (i != NULL)
    {
...
void ll_insert_before(linked_list_t *list, linked_node_t *before, elements *data)//insérer une data
{
    assert(list != NULL);
    assert(data != NULL);
...
void ll_insert_heuristic(linked_list_t *list, elements *data)//insérer par rapport à l'heuristic
{
    assert(list != NULL);
    assert(data != NULL);
    const int heuristic = data->heuristic;
    if (NULL == list->head || heuristic <= list->head->data->heuristic)
    {
        ll_prepend(list, data);
        return;
    }
    ...
    {
        // si l'heuristique est plus basse que celle du noeud, on insert avant le noeud
