...
void spawn_scanf(int map[MAP_HEIGHT][MAP_WIDTH], points *p_spawn, points chests_coord[4])
{ // get spawn point value
    ...
    do {
        ...
        good_coord = true;
        for (int i = 0; i < 4; i++) {
            if (p_spawn->x == chests_coord[i].x && p_spawn->y == chests_coord[i].y) { // on vérifie que le spawn n'est pas sur un coffre
                good_coord = false;
                break;
            }
        }
...
bool astar(int map[MAP_HEIGHT][MAP_WIDTH], points origin, points destination, array_t *found_path)
{
    ...
    for (int x = 0; x < MAP_WIDTH; x++) {
        for (int y = 0; y < MAP_HEIGHT; y++) { // on init les poids,  coord etc
    ...
    while (ll_is_empty(open_list) == false) {
        ...
        if (point_equals(u->coord, destination)) { // algo termine car on est au bon point
